...
#########################################
### Laser-Intensität Vergleich C001   ###
#########################################
dfs = []
for f in sorted(daten_dir.glob("2_*.dat")):
    # Intensitätsreduktion aus Dateiname extrahieren: "2_50p.dat" -> 50
    intensity_str = f.stem.split("_")[1]              # "50p"
    reduction_pct = int(intensity_str.replace("p", ""))  # 50
    df_tmp = read_drs4(f)
...
if not dfs:
    raise FileNotFoundError("Keine passenden 2_*.dat-Dateien in 'daten/' gefunden")
...
ax.set_title("Laser-Intensitätsvergleich")
ax.legend(title="Reduktion")
ax.grid(True)
plt.tight_layout()
plt.savefig("Auswertung/laser_intensitaet_vergleich.png", dpi=150)
...
# Ladung pro Event, nur Kanal C001, erste 1000 Events
...
ax.set_title("Gemittelte Ladung vs. Laser-Intensität")
ax.grid(True)
plt.tight_layout()
plt.savefig("Auswertung/charge_vs_laser.png", dpi=150)
...
plt.suptitle("9 zufällige Signale", fontsize=11)
plt.tight_layout()
plt.savefig("Auswertung/laser_random_signals.png", dpi=150)
...
ax.set_title("Gemittelte Signaldauer vs. Laser-Intensität ")
ax.grid(True)
plt.tight_layout()
plt.savefig("Auswertung/signalzeit_vs_laser.png", dpi=150)
